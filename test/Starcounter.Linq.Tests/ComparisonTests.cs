...
namespace Starcounter.Linq.Tests
{
    public class ComparisonTests
    {
        ...
        {
            Assert.Equal("SELECT E FROM \"Starcounter\".\"Linq\".\"Tests\".\"Employee\" E WHERE ((E.\"Department\" IS NOT NULL))",
                Sql(() => Objects<Employee>().Where(p => p.Department != null)));
        }
        ...
        {
            Assert.Equal("SELECT P FROM \"Starcounter\".\"Linq\".\"Tests\".\"Person\" P WHERE (NOT (P.\"Name\" = ?))",
                Sql(() => Objects<Person>().Where(p => !(p.Name == "XXX"))));
        }
